...
function storeTupleArray(source: Array) {
  let builder = new TupleBuilder();
  builder.writeCell(
    source.map.size > 0 ? beginCell().storeDictDirect(source.map, Dictionary.Keys.Uint(16), Dictionary.Values.BigInt(257)).endCell() : null
  );
...
export function storeTupleExtMessage(source: ExtMessage) {
  let builder = new TupleBuilder();
